...
export default function ActivityLogPage() {
  ...
  // Aggregate all activity logs from all lead measures
  const allActivityLogs = (wigs || []).flatMap((wig: WIG) =>
    wig.leadMeasures.flatMap((lm: LeadMeasure) =>
      (lm.activityLogs || []).map((log: ActivityLogEntry) => ({
        ...log,
        leadMeasureId: lm.id,
  ...
  const sortedLogs = allActivityLogs.sort((a: AggregatedActivityLog, b: AggregatedActivityLog) => new Date(b.loggedForDate).getTime() - new Date(a.loggedForDate).getTime());

  // Get all unique lead measures for dropdown
  const allLeadMeasures = (wigs || []).flatMap((wig: WIG) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await logActivity({
        ...
        loggedForDate: logDate as any, // ISO string or Date accepted by backend
        note: note || undefined,
      } as any);
      // Reset form
    ...
    <main style={{ flex: 1, padding: "32px", fontFamily: "'Inter', sans-serif" }}>
          ...
          <div style={{ width: "100%", maxWidth: "560px", border: "1px solid #e4e4e7", backgroundColor: "#ffffff", padding: "20px", display: "flex", flexDirection: "column", gap: "24px" }}>
            ...
            <div>
              ...
              <div style={{ position: "relative" }}>
                ...
                >
                  ...
                  {allLeadMeasures.map((lm: { id: string; name: string; wigTitle: string }) => (
            ...
            >
              <span className="material-symbols-outlined" style={{ fontSize: "20px" }}>
                {isSubmitting ? "hourglass_empty" : "add_task"}
              </span>
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <div style={{ width: "100%", maxWidth: "900px", marginTop: "48px", marginBottom: "48px" }}>
          ...
          {isLoading ? (
            <div style={{ padding: "48px", textAlign: "center", color: "#71717a" }}>Loading activity logs...</div>
          ) : sortedLogs.length === 0 ? (
            ...
            <div style={{ border: "1px solid #e4e4e7", backgroundColor: "#ffffff", overflow: "hidden" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {sortedLogs.map((log: AggregatedActivityLog, i: number) => (
                    <tr
                      key={i}
